...
class SurveyParser
	...
	def ranked_flavors
		...
		sorted_array.each.with_index do |x,i|
			sorted_array[i].last == sorted_array[i-1].last ? x.unshift(sorted_array[i-1].first) : x.unshift(i+1)
		end		
